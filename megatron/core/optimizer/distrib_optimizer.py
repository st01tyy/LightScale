...
class DistributedOptimizer(MixedPrecisionOptimizer):
    ...
    def _copy_model_grads_to_main_grads(self):
        ...
        print("======= _copy_model_grads_to_main_grads triggered =======")
        """
    ...
    def accumulate_grad_step(self, unscale: bool = False) -> bool:
        """
        Accumulate model grads into main grads for one microbatch.

        Returns:
            bool: Whether any non-finite values were detected during unscale.
        """
        if self.is_stub_optimizer:
            return False

        if self.ddp_config.use_custom_fsdp:
            # Custom FSDP path currently not supported in accumulation.
            return False

        def acc_group_grads(model_groups, shard_main_groups):
            for model_group, shard_main_group in zip(model_groups, shard_main_groups):
                for model_param, shard_main_param in zip(model_group, shard_main_group):
                    ...
                    if self.config.use_precision_aware_optimizer_no_fp8_or_ds_fp8:
                        existing = getattr(shard_main_param, "decoupled_grad", None)
                        if existing is None:
                            shard_main_param.decoupled_grad = shard_model_grad.clone()
                        else:
                            with torch.no_grad():
                                existing.add_(shard_model_grad)
                    else:
        ...
        if self.config.use_precision_aware_optimizer_no_fp8_or_ds_fp8:
            acc_group_grads(self.model_float16_groups, self.shard_float16_groups)
            acc_group_grads(self.model_fp32_groups, self.shard_fp32_groups)
        else:
            acc_group_grads(self.model_float16_groups, self.shard_fp32_from_float16_groups)
            acc_group_grads(self.model_fp32_groups, self.shard_fp32_groups)

        return False

    def _copy_main_params_to_model_params(self):
        print("======= _copy_main_params_to_model_params triggered =======", flush=True)
        """
    ...
    @torch.no_grad()
    def step_with_accumulated_grads(self):
        timers = self.config.timers
