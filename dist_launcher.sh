...
for (( LOCAL_RANK=0; LOCAL_RANK<$NPROC_PER_NODE; LOCAL_RANK++ )); do
    ...
    LAUNCH_PREFIX=()
    # if [ $GLOBAL_RANK -eq 95 ]; then
    #     echo "WARNING: enabling nsys profiling"
    #     LAUNCH_PREFIX=(
    #         nsys profile
    ...
    #         -o "/root/work/externalstorage/gpfsprd/taoyuyang/mrl_gkd/nsys_profiling_output/32k_rl_low_tps_split_stage_last_pp"
    #     )
    # fi
    RANK="$GLOBAL_RANK" LOCAL_RANK="$LOCAL_RANK" "${LAUNCH_PREFIX[@]}" $EXEC_CMD $EXEC_ARGS > "${LOG_DIR}/rank_${GLOBAL_RANK}.log" 2>&1 &
    PIDS+=($!)
done
