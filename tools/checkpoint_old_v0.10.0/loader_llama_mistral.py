...
def add_arguments(parser):
    ...
    parser.add_argument('--model-size', type=str, required=True,
                        choices=['llama2-7B', 'llama2-13B', 'llama2-70B', 'llama2-7Bf', 'llama2-13Bf', 'llama2-70Bf', 'llama3-8B', 'llama3-70B', 'llama3-8Bf', 'llama3-70Bf', 'mistral-7B', 'mistral-7Bf', 'yi-34B', 'qwen2.5-7B', 'qwen2.5-72B', 'qwen2.5-7Bf', 'qwen2.5-32B', 'qwen2.5-14B', 'qwen2.5-72Bf'],
...
def load_args_from_checkpoint(args, model_size):
    ...
    if "qkv_bias" in model_args:
        args.add_qkv_bias = model_args["qkv_bias"]
    else:
    ...
    args.attention_dropout = model_args["attention_dropout"]
    args.hidden_dropout = model_args.get("hidden_dropout", 0.0)
...
def _load_checkpoint(queue, args):
    ...
    margs = parse_args() # megatron training arguments
    margs.tokenizer_model = args.tokenizer_model # TODO: hf tokenizer 如何设置
    load_args_from_checkpoint(margs, model_size=args.model_size)
    ...
    margs.use_legacy_models = True  # why?
    ...
    print(f"param dtype: {margs.params_dtype}")
    ...
    # set this no use, must set to margs as checkpoint_args #568
    # md.attention_dropout = 0.0
    # md.hidden_dropout = 0.0

    margs.model_size = args.model_size

    # Get true (non-padded) vocab size
    # tokenizer = transformers.AutoTokenizer.from_pretrained(margs.tokenizer_model, trust)
    hf_config = transformers.AutoConfig.from_pretrained(args.load_dir, trust_remote_code=True) # this is the right way
    md.true_vocab_size = hf_config.vocab_size
