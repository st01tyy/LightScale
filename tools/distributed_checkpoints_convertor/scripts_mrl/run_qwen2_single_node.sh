...
read_model_config() {
    ...
    QKV_BIAS=$(jq -r '.qkv_bias // false' "$config_file")
...
if [ -n "$PP_FIRST_LAYERS" ]; then 
    MODEL_PARALLEL_ARGS+=(--decoder-first-pipeline-num-layers "${PP_FIRST_LAYERS}")
fi

if [ -n "$PP_LAST_LAYERS" ]; then
    MODEL_PARALLEL_ARGS+=(--decoder-last-pipeline-num-layers "${PP_LAST_LAYERS}")
fi
...
if [ ${MG2HF} = true ]; then
    ...
    mkdir -p ${SAVE_DIR}
    find -L ${HF_DIR} -maxdepth 1 -type f -name "*.json" -print0 | xargs -0 -r cp -t ${SAVE_DIR}
    find -L ${HF_DIR} -maxdepth 1 -type f -name "merges.txt" -print0 | xargs -0 -r cp -t ${SAVE_DIR}
    find -L ${HF_DIR} -maxdepth 1 -type f -name "*.jinja" -print0 | xargs -0 -r cp -t ${SAVE_DIR}
    find -L ${HF_DIR} -maxdepth 1 -type f -name "*.py" -print0 | xargs -0 -r cp -t ${SAVE_DIR}
else
    OTHER_ARGS+=(
        --tokenizer-type HuggingFaceTokenizer
...
# GPT model arguments - dynamically built from config.json
# TODO: 可配置的qkv bias
GPT_MODEL_ARGS=(
